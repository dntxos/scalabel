...
func loadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if env.UserManagement == "on" || env.UserManagement == "On" || env.UserManagement == "ON" {
		...
		// redirect to create
		createUrl := "http://dev.gerabytes.com.br:8686/create"
...
func authHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || !accessToken.Valid {
		...
		newUrl := "http://dev.gerabytes.com.br:8686/"
		...
		if err != nil || !idToken.Valid || identity == "" {
			...
			*/
			...
			if identity == "worker" {
				...
				newUrl := "http://dev.gerabytes.com.br:8686/workerDashboard"
				http.Redirect(w, r, newUrl, 301)
				return
			} else if identity == "admin" {
				// if the user is admin, redirect to admin's dashboard
				Info.Println("Admin's Cookie Saved")
				newUrl := "http://dev.gerabytes.com.br:8686/adminDashboard"
				http.Redirect(w, r, newUrl, 301)
				return
			}
		}
...
func logOutHandler(w http.ResponseWriter, r *http.Request) {
	...
	logOutUrl := fmt.Sprintf("https://satworker.auth.%v.amazoncognito.com/logout?client_id=%v&logout_uri=http://dev.gerabytes.com.br:8686/", env.Region, env.ClientId)
